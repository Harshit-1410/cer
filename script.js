...
// Tile calculator functionality with feet
function calculateTiles() {
    const length = parseFloat(document.getElementById('roomLength').value); // in feet
    const width = parseFloat(document.getElementById('roomWidth').value);   // in feet
    ...
    setTimeout(() => {
        // Convert feet to meters for area calculation
        ...
        const tilesNeeded = Math.ceil(areaM2 / tileArea);
        const wastage = Math.ceil(tilesNeeded * 0.05); // 5% wastage
        const totalTiles = tilesNeeded + wastage;

        // Update result display
        document.getElementById('roomArea').textContent = areaFt2.toFixed(2) + ' sq ft';
